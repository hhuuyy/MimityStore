...
public class RevenueController {
	...
	public String byQuater(ModelMap model) {
		...
		String hql = "SELECT " +
				"	Cast(Ceiling(month(d.order.orderDate)/3.0) as integer) as Q," +
				"	SUM(d.quantity), " +
	...
	public String byYear(ModelMap model) {
		...
		String hql = "SELECT " +
				"	year(d.order.orderDate) as YR," +
				"	SUM(d.quantity), " +
				"	SUM(d.quantity*d.unitPrice*(1-d.discount)) as DS, " +
				"	MIN(d.unitPrice), " +
				"	MAX(d.unitPrice), " +
				"	AVG(d.unitPrice)" +
				" FROM OrderDetail d " +
